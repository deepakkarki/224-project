...
//Creates an ObjectEntry with context having a clock of time stamp 'ts' and dummy data
func MakeObjectEntry(ts map[string]uint) mydynamo.ObjectEntry {
	ctx := mydynamo.NewContext(mydynamo.PresetVectorClock(ts))
	return mydynamo.ObjectEntry{Context: ctx, Value: []byte("val")}
}

func ObjectEquals(o1, o2 mydynamo.ObjectEntry) bool {
	ts1 := o1.Context.Clock.GetTimestamp()
	ts2 := o2.Context.Clock.GetTimestamp()
